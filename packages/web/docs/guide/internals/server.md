...
5. **Run `onBoot`**: If configured, awaits the boot callback inside a `Promise.race` with a configurable timeout (`maxWaitTime`, default 10s). On success, logs `Booted in Xms`. On failure or timeout, logs via diagnostics logger and calls `process.exit(1)`. The function does not return until boot completes. Boot timing is always visible via `console.log`
